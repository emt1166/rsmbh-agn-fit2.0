...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  5 13:15:42 2024
@author: emilytemple

We need to have a way of estimating the errors that we get from this fitting 
process. 
Right now, the MC ans MCMC are not feasible...not really working for us. 

I have already used the SDSS Uncertainy to refit the sample multiple times.

I want to use this script to fit the true flux data many times and see if there
is anything interesting from those results. I would hope that if the results are 
quite different from iteration to iteration that this would be reflected in the
SDSS Uncertainty iterarions...

A lot of this code will be taken from the original pyqso scipts. 
I will modify it to run with multiple iterations and save those measurements in 
a master file, organized by Object ID and measurement type.
"""

#import statements and set up the code
import os,timeit
import numpy as np
...
# save fits path, unused but kept just in case
path3 = path1+'/Fit Results/QA Other'
# The path of the Data files to be used 
pathD = '/Users/emilytemple/documents/rsmbh-agn-fit2.0/pyqsofit/Data/Outliers'
...
#defining the function to iterate over the outliers 
def Fit_Function(sourcename):
    ...
    path2 = path1+'/Fit results/Line Properties/Outliers Iterations/'+sourcename+'/'
    
    if not os.path.exists(path1+'/Fit results/Line Properties/Outliers Iterations/'+sourcename):
        os.makedirs(path1+'/Fit results/Line Properties/Outliers Iterations/'+sourcename)
    ...
    # Allowing iterations of Outlier fits...
    # Note: NOTHING is changing between iterations for this...
    
    n = 0 
    for n in range(2): #range tells you how many times to fit the spectrum
    ...
    # Do the fitting. NOTE: Change any arguments accordingly
        ...
        q_mle.Fit(name=None, nsmooth=1, and_mask=False, or_mask=False, reject_badpix=True, deredden=True,
              ...
              epsilon_jitter=0., save_result=True, save_fits_name=None, save_fits_path=path3,  
        ...
        if MCMC == True:
            q_mcmc = QSOFit(lam, flux, err, z, ra=ra, dec=dec, plateid=plateid, mjd=mjd, fiberid=fiberid, path=path1)
        ...
        #path5 = pathO+sourcename+'/'
         ...
         # NOTE 2: If you are using MCMC or MC, the code must be adjusted. 
        ...
        if MCMC != True:
            ...
            # IF USING THE MCMC RESAMPLING METHOD --------------------------------------------------
            # name needs to change to q_mcmc not q_mle, if not MCMC, it will do the plotting below
            ...
            for p in range(int(len(gauss_result) / 3)):
                if q_mcmc.CalFWHM(gauss_result[3 * p + 2]) < 1000:  # < 1000 km/s narrow
                    color = 'g'  # narrow
                else:
                    color = 'r'  # broad
        ...
        bl_profile = np.load(path+sourcename+'_BLSpectrum.npy') #cleaned data!
        ...
        # MUCH easier
        # BL Data
        bl_matrix = np.vstack((wavelength, bl_profile)).T #changed bl to bl_profile to use cleaned data (?)
        ...
        # .txt file, n indicates the iterations done 
...
# We want to loop these fits and see if anything is super unstable...
loop = False
start_time = time.time()

if loop:
    for source in Data_list[:1]: #start with first 20, 5 iterations each
        Fit_Function(source)
else:
    Fit_Function('1814-54555-0405')
end_time=time.time()
