...
pathF = '/Users/emilytemple/documents/rsmbh-agn-fit2.0/pyqsofit/Data/Outliers New Flux/TEST/'
    ...
    for n in range(5):
        ...
        while i < len(flux):
            ...
            if i > len(flux):
                break
            i +=1 
        np.savetxt(pathF+sourcename+'.'+f'{n}'+'new_flux.txt', new_flux)
        
    
Input_SN('2220-53795-0607') #this portion works great 
 # ------------------------------------------------------------------------------------------   
 #loop over a single source for original spec, plus SN input spec = 6
...
def Fit_Function(sourcename):
    ...
    flux = data[1].data['flux']                           # OBS flux (erg/s/cm^2/A)
    n = 2
    new_flux = np.loadtxt(pathF+sourcename+'.'+f'{n}'+'new_flux.txt')
    ...
    with open(path2+sourcename+"_LineProperties.txt","w") as f:
