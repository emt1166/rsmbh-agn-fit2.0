"""
LineProfile_Analysis.py
L. Matrecito, lmm8709@rit.edu
Mon Apr 29 2024

This code can analyzes the fitting results of the python code Fitting-Script.py 
...
#Specify which data you want to loop through
#this will automatically get the source names for you
...
for name in Data_list_names:
 ...
 # EMT EDIT FOR A SPECIFIC .CSV FILE OF SPECTRA -------------------------------------

data_csv = pd.read_csv('/Users/emilytemple/documents/rsmbh-agn-fit2.0/pyqsofit/Data/SubsetOutliers_Noisy.csv')

 # Filter the Data based on the number sequences in the .csv file!
filtered_data = data_csv[data_csv['NumberSequence'].isin(Data_list)]
 # Skip the first row (containing column names) using iloc
filtered_data = filtered_data.iloc[1:]

 #print(filtered_data)       
        
    
# ADDING IN WRAPPER FUNCTION TO LOOP THROUGH THE ENTIRE DIRECTORY

def lineprofile_analysisfunc(source):
    ...
    #source = '0332-52367-0639'
    SDSS_name = ''
    
    #EMT EDIT -------------------------------------------------------------------------------
    #getting information for the spectrum, as needed
    #this is really for the redshift
    data = fits.open(os.path.join(Data_path+'/'+'spec-'+source+'.fits'))
    ...
    path = '/Users/emilytemple/documents/rsmbh-agn-fit2.0/pyqsofit/Fit Results/Outliers_Noisy/'+source+'/Fit Data/'
    # Path for saving results from this code
    path2 = '/Users/emilytemple/documents/rsmbh-agn-fit2.0/pyqsofit/Fit Results/Outliers_Noisy/'+source+'/'
    ...
    def vs_calcs(wave_range, nl_range, bl_data, nl_data, wavelength, bl, nl, 
        ...
        # separate .txt file
        # !!!! EMT EDITED FORMATTING based on code to read the output files of the entire dataset !!!!
...
#         lineprofile_analysisfunc(source)
# else:
#     lineprofile_analysisfunc('0332-52367-0639')

# ----------------------------------------------------------------------------------------
#Below is for the modified version where I am iterating over a dataframe 
#not the directory 
loop = True

if loop:
    for index, row in filtered_data.iterrows():
        source = row['NumberSequence']  # the actual column name
        lineprofile_analysisfunc(source)
else:
    lineprofile_analysisfunc('0273-51957-0008')    
